...
export default function NewsPage() {
  ...
  return (
    <main className="min-h-screen bg-[var(--bg-main)]">
      ...
      <section className="max-w-7xl mx-auto px-4 md:px-10 pt-20 pb-10 border-b border-[var(--border-soft)]">
      ...
      <section className="max-w-7xl mx-auto px-4 md:px-10 py-8">
        <div className="flex flex-col gap-6">
          <div className="flex flex-col md:flex-row gap-3">
            <div className="flex-1 relative">
              ...
              <input
                ...
                className="w-full pl-11 pr-4 py-3 text-sm border border-[var(--border-soft)] focus:border-black outline-none bg-white transition-colors rounded-lg"
              />
            </div>
            <div className="relative">
              <button
                onClick={() => setShowSources(!showSources)}
                className="flex items-center gap-2 px-4 py-3 bg-white border border-[var(--border-soft)] rounded-lg hover:border-black transition-all text-sm"
              >
                <Filter size={14} className="text-[var(--text-secondary)]" />
                {activeSource === "all" ? "All Sources" : activeSource}
                <ChevronDown size={14} className={`text-[var(--text-secondary)] transition-transform ${showSources ? "rotate-180" : ""}`} />
              </button>
              {showSources && (
                <div className="absolute top-full right-0 mt-2 w-72 bg-white border border-[var(--border-soft)] rounded-lg shadow-[0_20px_50px_rgba(0,0,0,0.12)] z-50 max-h-80 overflow-y-auto">
                  <div
                    onClick={() => { setActiveSource("all"); setShowSources(false); }}
                    className={`flex items-center justify-between px-4 py-3 cursor-pointer hover:bg-gray-50 text-sm border-b border-[var(--border-soft)] ${activeSource === "all" ? "font-semibold" : ""}`}
                  >
                  ...
                  {sources.map((s) => (
                    <div
                      ...
                      className={`flex items-center justify-between px-4 py-3 cursor-pointer hover:bg-gray-50 border-b border-[var(--border-soft)] last:border-0 ${activeSource === s.name ? "font-semibold" : ""}`}
                    >
                      <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-6 overflow-x-auto no-scrollbar pb-1 border-b border-[var(--border-soft)]">
            {CATEGORIES.map((cat) => (
              <button
                key={cat.id}
      ...
      <section className="max-w-7xl mx-auto px-4 md:px-10 pb-20">
        {loading ? (
          ...
          <div className="text-center py-32">
            ...
            <button onClick={fetchNews} className="btn-primary text-xs">
              Retry
            </button>
          </div>
          ...
          <div className="text-center py-32">
            ...
            <p className="text-sm font-semibold text-[var(--text-secondary)]">No stories found</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredItems.map((item, i) => (
              ...
              >
                ...
                <h3 className="text-lg font-semibold tracking-tight leading-snug line-clamp-3 group-hover:text-[var(--text-secondary)] transition-colors">
                  {item.title}
                </h3>
                ...
                <div className="flex items-center justify-between mt-auto pt-4 border-t border-[var(--border-soft)]">
                  <a
                    ...
                    className="flex items-center gap-1.5 text-xs text-[var(--text-secondary)] hover:text-black transition-colors"
                  >
