...
const WIDTH = 1200;
const HEIGHT = 630;
// Единственный реально живой гейтвей — pinata.cloud, и он медленный: живой
// баннер тянется 3-10с (замерено: до 3.8с на одном из свежих CID, остальные
// гейтвеи отдают пустое тело или падают сразу). Старый таймаут в 4с не успевал
// смержиться и роут всегда уходил в fallback-карточку, хотя браузер показывал
// баннер (у <img> нет таймаута). Теперь ждём как браузер + ретрай.
const GATEWAY_TIMEOUT_MS = 15000;
const GATEWAY_RETRY_DELAY_MS = 1000;
const GATEWAY_MAX_ROUNDS = 2;

/**
 * Минималистичная fallback-карточка (градиент + только заголовок поста).
 ...
 * баннера). Без брендинга, логотипов и сервисных надписей — только суть.
 * Построена на чистом SVG — не может упасть, в отличие от кадрирования
 * бинарной картинки.
 */
...
function buildFallbackSvg(rawTitle: string): string {
  ...
  const lines = wrapTitle(clean, maxChars);
...
async function fetchSourceBuffer(url: string): Promise<Buffer | null> {
  ...
  // живого гейтвея, а баг выбран по исходному порядку.
  //
  // Pinata медленный — один баннер идёт 3-10с. Вместо жёсткой сдачи после
  // одного раунда (как с прежним 4-секундным таймаутом) делаем до двух раундов
  // опроса: свежий CID может ещё «поехать» на гейтвеях через секунду-другую,
  // ровно как браузерный <img> с его ретраями и рециклом гейтвеев.
  const order = new Map(variants.map((v, i) => [v, i]));

  for (let round = 0; round < GATEWAY_MAX_ROUNDS; round++) {
    const attempts = await Promise.allSettled(
      variants.map(async (variant) => {
        const res = await fetch(variant, { signal: AbortSignal.timeout(GATEWAY_TIMEOUT_MS) });
    ...
    if (success.length > 0) return success[0].buffer;
    if (round < GATEWAY_MAX_ROUNDS - 1) {
 ...
 * Без оверлея и логотипов — только чистый кроп баннера под канвас OG.
 */
